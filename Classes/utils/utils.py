...
def make_symmetric(matrices):
    N, A, B = matrices.shape
    # Ensure the matrices are square for symmetry
    if A != B:
        raise ValueError("Matrices must be square (A == B) for symmetry.")
    
    for n in range(N):
        upper_triangle = np.triu(matrices[n])
        matrices[n] = upper_triangle + upper_triangle.T - np.diag(upper_triangle.diagonal())
    return matrices
...
def convert_pdfs_to_images(params, prefix, evaluation_epochs, convert = 1):
    ...
    #             image.save(os.path.join(images_path, f'{prefix}_{epoch}_{i}.jpg'), 'JPEG')
...
def normalize_numpy(array, min_or_mean = None, max_or_std = None, normalize = 1, type_='minmax', already_centered = 0):

    if isinstance(array, torch.Tensor):
        min_or_mean = return_tensor(min_or_mean)
        max_or_std = return_tensor(max_or_std)
    elif isinstance(array, np.ndarray):
        min_or_mean = return_numpy(min_or_mean)
        max_or_std = return_numpy(max_or_std)

    if already_centered:
        min_or_mean = np.zeros_like(min_or_mean)

    if normalize == 1:
        if type_=='minmax':
            array = (array-min_or_mean)/(max_or_std-min_or_mean)
        elif type_=='standardize':
            array = (array-min_or_mean)/max_or_std
    else:
        if type_=='minmax':
            array = array*(max_or_std-min_or_mean) + min_or_mean
        elif type_=='standardize':
            array = array * max_or_std + min_or_mean
